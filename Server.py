...
def clientThread(clientSocket, clientAddress):
    ...
    while True:
        ...
        print(clientAddress[0] + ":" + str(clientAddress[1]) + user.getUser(hostIp) + message)
        for client in clients:
            if client is not clientSocket:
                client.send((clientAddress[0] + ":" + str(clientAddress[1]) + user.getUser(hostIp)+ message).encode("utf-8"))
